...
module.exports = {
  ...
  startNet: function(req,res) {
    ...
    db.query('SELECT * FROM neuralNet', function(err, response) {
      if(err) {
        ...
        console.log('testing data looks like this');
  ...
  testBrain: function(testData) {
    ...
    for(var i = 0; i < testData.length; i++) {
      if (i===0) console.log(testData[i]);
      var nnPredictions = {defaulted: net.run(testData[i].input).defaulted};
      testData[i].nnPredictions = nnPredictions;
      // if (i===0) { console.log(testData[i]) }
    ...
    console.log('trained data looks like:')
